...
export default function CapacityReports({ scopes }: { scopes: Scope[] }) {
  ...
  return <section aria-labelledby="capacity-heading">
    ...
    {run && <>
      <dl className="facts run-identity"><dt>Selected-domain projection</dt><dd>{currentContext().selected_domain}</dd><dt>Pinned run</dt><dd><code>{run.id}</code></dd><dt>Saved at</dt><dd>{run.created_at}</dd><dt>Demo clock</dt><dd>{run.demo_clock_at}</dd><dt>Ledger revision</dt><dd>{run.ledger_version}</dd></dl>
