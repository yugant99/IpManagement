...
function FindingDetail({ runId, findingId, scopes, onClose }: { runId: string; findingId: string; scopes: Scope[]; onClose: () => void }) {
  ...
  return <aside className="detail-panel" id="finding-detail" aria-labelledby="finding-heading" aria-busy={finding.status === "loading"}>
    ...
    {finding.status === "ready" && <>
      ...
      <section className="detail-section"><h3>Selected intended policy</h3>{finding.data.policy ? <><dl className="facts compact"><dt>Announcement expected</dt><dd>{finding.data.policy.expects_announcement ? "Yes" : "No"}</dd><dt>Route match</dt><dd>{finding.data.policy.route_match_policy === "exact" ? "Exact prefix" : "Exact or covering prefix"}</dd><dt>Effective from</dt><dd><time>{finding.data.policy.effective_from_at}</time></dd></dl><details className="provenance"><summary>Stored policy and references</summary><JsonValue value={finding.data.policy} /></details></> : <p>No applicable policy was selected. No expectation is inferred from inventory alone.</p>}</section>
      <section className="detail-section"><h3>Selected routing coverage</h3><CoverageList coverage={finding.data.coverage} scopes={scopes} /></section>
      <section className="detail-section"><h3>Limitations</h3>{finding.data.limitations.length ? <ul className="limitation-list">{finding.data.limitations.map((item, index) => <li key={index}>{item}</li>)}</ul> : <p>No additional limitations recorded for this finding.</p>}<p className="quiet">Unknown is insufficient evidence. Not applicable means the policy does not require an announcement. Neither is a healthy routing claim.</p></section>
      <section className="detail-section"><h3>Input provenance</h3><details className="provenance"><summary>Intended inventory origin</summary><JsonValue value={finding.data.subject.origin} /></details>
        ...
        {finding.data.input_references.map((reference, index) => <article className="inventory-record" key={`${reference.record_id ?? reference.batch_id ?? reference.source_id}-${index}`}><h4>{reference.kind.replaceAll("_", " ")}</h4><dl className="facts compact"><dt>Source</dt><dd><code>{reference.source_id}</code></dd><dt>Source run</dt><dd><code>{reference.source_run_id}</code></dd>{reference.batch_id && <><dt>Batch ID</dt><dd><code>{reference.batch_id}</code></dd></>}{reference.source_record_id && <><dt>Source record</dt><dd><code>{reference.source_record_id}</code></dd></>}</dl>{reference.record_id && <button className="text-button" onClick={() => setRawId(reference.record_id!)}>Load stored raw record</button>}</article>)}
...
function RunResults({ run, scopes }: { run: SavedRun; scopes: Scope[] }) {
  ...
  return <section className="run-results" aria-labelledby="run-heading">
    <div className="section-heading"><h3 id="run-heading">Saved calculation result</h3><span className="quiet">Synthetic · Missing expected route only</span></div>
...
export default function FirstPath({ scopes }: { scopes: Scope[] }) {
  ...
  return <div className="first-path">
    <div className="page-heading"><div><p className="eyebrow">Source evidence → saved calculation</p><h1>Missing-route evidence</h1><p className="intro">Import synthetic routing and intended policy, compute one rule, and inspect the evidence.</p></div></div>
    ...
    <section className="path-step" aria-labelledby="import-heading"><div className="step-heading"><span className="step-number" aria-hidden="true">1</span><div><h2 id="import-heading">Import a source envelope</h2><p className="quiet">Choose one routing or intended-policy JSON file. Inventory and allocations stay unchanged.</p></div></div>
    ...
    <section className="path-step" aria-labelledby="compute-heading"><div className="step-heading"><span className="step-number" aria-hidden="true">2</span><div><h2 id="compute-heading">Compute missing expected routes</h2><p className="quiet">The API saves a new run using the current intended ledger and selected source batches.</p></div></div>
      <div className="compute-actions"><button disabled={runBusy !== null || importing} onClick={() => { void loadRun(); }}>{runBusy === "computing" ? "Computing and saving…" : "Compute missing-route rule"}</button><span className="quiet">Explicit trigger · One rule · No network changes</span></div>
      ...
      {!run && !runBusy && <div className="empty-state"><h3>No saved result selected</h3><p>Compute the rule or open a saved run. The browser does not calculate findings or infer healthy state from an empty list.</p></div>}
