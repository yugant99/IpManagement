  ...
  return <aside className="detail-panel" id="finding-detail" aria-labelledby="finding-heading" aria-busy={finding.status === "loading"}>
    ...
    {finding.status === "ready" && <>
      ...
      <section className="detail-section"><h3>Input provenance</h3><details className="provenance"><summary>Intended inventory origin</summary><JsonValue value={finding.data.subject.origin} /></details>
        ...
        {finding.data.input_references.map((reference, index) => <article className="inventory-record" key={`${reference.record_id ?? reference.batch_id ?? reference.source_id}-${index}`}><h4>{reference.kind.replaceAll("_", " ")}</h4><dl className="facts compact"><dt>Source</dt><dd><code>{reference.source_id}</code></dd><dt>Source run</dt><dd><code>{reference.source_run_id}</code></dd>{reference.batch_id && <><dt>Batch ID</dt><dd><code>{reference.batch_id}</code></dd></>}{reference.source_record_id && <><dt>Source record</dt><dd><code>{reference.source_record_id}</code></dd></>}{reference.audit_id && <><dt>Matching metadata audit</dt><dd><code>{reference.audit_id}</code> · Inspect this prefix's audit in Requests and exceptions.</dd></>}</dl>{reference.record_id && <button className="text-button" onClick={() => setRawId(reference.record_id!)}>Load stored raw record</button>}</article>)}
