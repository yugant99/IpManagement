...
function SourceCatalog({ scopes, revision, onSelect }: { scopes: Scope[]; revision: number; onSelect: (id: string) => void }) {
  const [offset, setOffset] = useState(0);
  const [catalogRevision, setCatalogRevision] = useState(0);
  const catalog = useApiResource<SourceCatalogResponse>(`/api/source-catalog?limit=${PAGE_SIZE}&offset=${offset}`, revision + catalogRevision);
  return <section className="inventory-panel" aria-labelledby="source-catalog-heading" aria-busy={catalog.status === "loading"}>
    <div className="section-heading"><div><h2 id="source-catalog-heading">Imported-source catalog</h2><p className="quiet">Receipt-derived source identity and per-scope coverage. This is not automatic discovery.</p>{catalog.status === "ready" && <p className="quiet">Evaluated at <time>{catalog.data.evaluated_at ?? "the seeded demo clock"}</time></p>}</div><button className="text-button" onClick={() => setCatalogRevision((value) => value + 1)}>Reload catalog</button></div>
    {catalog.status === "loading" && <p className="panel-message" role="status">Loading source catalog…</p>}
    {catalog.status === "error" && <ErrorNotice error={catalog.error} retry={() => setCatalogRevision((value) => value + 1)} />}
    {catalog.status === "ready" && <><div className="table-scroll" tabIndex={0} role="region" aria-label="Imported source catalog"><table><caption className="sr-only">Imported sources selected at the seeded demo clock.</caption><thead><tr><th>Source / run</th><th>Scope</th><th>Authority / status</th><th>Freshness / completeness</th></tr></thead><tbody>{catalog.data.items.map((item) => <tr key={`${item.batch_id}-${item.scope_id ?? "staged"}`}><td><button className="prefix-link" onClick={() => onSelect(item.batch_id)}>{item.source_name ?? item.source_id}</button><div className="table-secondary mono">{item.source_id} · {item.source_run_id}</div><div className="table-secondary">{item.source_kind} · batch <code>{item.batch_id}</code></div></td><td>{item.scope_id ? scopes.find((scope) => scope.id === item.scope_id)?.name ?? item.scope_id : "Not applicable — staged candidate"}</td><td>{item.authority} · {item.authority_status}<div className="table-secondary">{item.application_status} · {item.rejection_status}</div></td><td>{item.freshness} · {item.completeness}<div className="table-secondary">{item.window_start_at ?? "No coverage window"}{item.window_end_at ? ` → ${item.window_end_at}` : ""}</div></td></tr>)}</tbody></table></div><Pagination page={catalog.data} onChange={setOffset} label="Imported source catalog pages" /></>}
  </section>;
}
...
function RunResults({ run, scopes }: { run: SavedRun; scopes: Scope[] }) {
  ...
  return <section className="run-results" aria-labelledby="run-heading">
    <div className="section-heading"><div><p className="eyebrow">Pinned calculation</p><h3 id="run-heading">Saved domain result</h3></div><span className="quiet">{run.rule_id}</span></div>
    ...
    <dl className="overview-counts" aria-label="Evidence states in this saved run"><div><dt>Evaluations in {currentContext().selected_domain} projection</dt><dd>{run.overview.total}</dd></div>{(Object.keys(stateLabels) as EvidenceState[]).map((key) => <div key={key}><dt><EvidenceBadge state={key} /></dt><dd>{run.overview[key]}</dd></div>)}</dl>
    ...
    <div className="result-toolbar" role="group" aria-label="Filter saved findings"><div className="result-toolbar-heading"><h4>Findings in this run</h4><p className="quiet">Evidence state and rule severity answer different questions.</p></div><div className="filters result-filters"><label>Network scope<select value={scope} onChange={(event) => { setScope(event.target.value); changePage(0); }}><option value="">All network scopes</option>{scopes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Evidence state<select value={state} onChange={(event) => { setState(event.target.value); changePage(0); }}><option value="">All evidence states</option>{(Object.keys(stateLabels) as EvidenceState[]).map((key) => <option value={key} key={key}>{stateLabels[key]}</option>)}</select></label><label>Rule<select value={rule} onChange={(event) => { setRule(event.target.value); changePage(0); }}><option value="">All rules</option>{[...new Set(run.findings.map((finding) => finding.rule_id))].sort().map((id) => <option key={id}>{id}</option>)}</select></label><label>Severity<select value={severity} onChange={(event) => { setSeverity(event.target.value); changePage(0); }}><option value="">All severities</option>{["critical", "high", "warning"].map((value) => <option key={value}>{value}</option>)}</select></label></div></div>
...
export default function FirstPath({ scopes, onAssess }: { scopes: Scope[]; onAssess?: (batchId: string) => void }) {
  ...
  return <div className="first-path">
    <div className="page-heading"><div><p className="eyebrow">Domain {currentContext().selected_domain} · saved projection</p><h1>Reconciliation</h1><p className="intro">Inspect source receipts, pinned calculations, and the evidence behind each finding.</p></div></div>
