...
function RunResults({ run, scopes }: { run: SavedRun; scopes: Scope[] }) {
  ...
  return <section className="run-results" aria-labelledby="run-heading">
    ...
    <p className="filter-help result-count" role="status">{findings.status === "ready" ? findings.data.total ? `Showing ${findings.data.offset + 1}–${findings.data.offset + findings.data.items.length} of ${findings.data.total} matching findings; ${run.overview.total} total evaluations in this saved run.` : `0 matching findings; ${run.overview.total} total evaluations in this saved run.` : findings.status === "loading" ? "Matching findings are loading." : "Matching findings are unavailable; see the error above."}</p>
