...
| **Stopped recovery** — FR-016; RFP-018/020/023 | RFP-018 **Demonstrated** for bounded stopped backup/restore; RFP-020 and RFP-023 **Partial** at their exact accepted candidates. Prepared-host timing and one Mac-to-Linux transfer do not establish full outage recovery, geo-DR or sustained hybrid operation. | Full incident boundary including diagnosis, artifact retrieval/transfer, replacement provisioning, restore/migration/startup, logical business checks, loss accounting and key/dependency access. | Terra for architecture/state gaps; Spencer for an approved operator procedure; Luna for independent evidence; Lead adjudicates. | Later authorized exact-candidate exercise that names start/end, excluded preparation, snapshot identity/age, acknowledged post-snapshot loss and preserved assignment/audit checks. Source SLA controls if stricter. |
