...
## PR #22 schema-v4 wording decision

[PR #22](https://github.com/yugant99/IpManagement/pull/22) at **`16c316d0afa5c22bccb5268eba5db113ada1680a`** is accepted as an **unmerged source checkpoint**. It targets `codex/stage-4-scheduler-integration`; exact base is the pushed intermediate schema/store checkpoint **`16160806e91023a29eae501b824c6b43e5e56d87`**. Core task `01a0b8ae-bd65-7331-b548-da5dca1e0d5e` owns its isolated `Ip_inventory-state-schema4-docs` worktree and `codex/part-1-state-schema4-docs` branch.

The lead and an independent reviewer read the complete two-file delta against the pinned shared store/schema and existing state operations. No actionable findings: `__main__.py` changes only migrate help to name legacy v1/v2/v3; dispatch/state algorithms are untouched. `STATE_OPERATIONS.md` correctly records current v4, version-scoped table/singleton checks, disabled/six-hour/cursor-zero migration defaults, whole-database restoration of clock/evidence/runs/schedule/replay, absence of keys newer than a restored snapshot, and separately supplied code/feed assets. Required future timer behavior is distinguished from implementation and runtime evidence.

The shared policy recognizes legacy versions 1–3 and current 4; existing state operations consume that policy. No concrete algorithm change was indicated by this bounded source review. This does not accept the complete Stage 4 implementation or demonstrate migration/backup/restore/restart behavior. No application/state/migration/interpreter/import/parser/type/smoke/test/build/browser/infrastructure command was run. Populated scheduler/replay preservation, refusal/failure outcomes, timer shutdown/restart and packaged asset availability remain unverified. `PART6_READY=no`; denominator 111 and demonstrated-row count are unchanged.

The lead permits normal history-preserving integration of **exact `16c316d0afa5c22bccb5268eba5db113ada1680a`** on Stage 4's isolated feature branch. Shared CLI conflicts return to core. This is source integration, not application/main acceptance; it may cause GitHub to mark PR #22 merged into its feature target after push. Record the actual resulting source/PR state at the next checkpoint. The lead does not merge this application-dependent PR into main.
