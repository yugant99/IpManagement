...
This is a generated pickup for the actual planning-to-build boundary, not a future-stage template. Planning is complete; the application is not implemented. Start a fresh lead chat for this stage and retain the planning chat for assigned follow-ups.
...
Start with `codex/part-1-foundation` from the current integration baseline. Use further feature branches for distinct work; a stage is not permission to combine unrelated features. The lead owns shared schema, startup, locks and contracts. A UI agent may own assigned `frontend/` files in an isolated worktree after payloads are fixed. A data agent may prepare the small `fixtures/` baseline after fields/IDs are fixed. Do not switch another agent's checkout or have two lanes regenerate shared locks.
...
2. Record Stage 1 as active, claim owned files and establish the actual backend/frontend installation and lockfile conventions. Keep FastAPI/Uvicorn, React/Vite, SQLite and standard IP address arithmetic; no stack shopping.
...
Stage 1 ends at a pushed foundation checkpoint with actual interfaces/files and explicit implemented-versus-unverified state. Do not quietly expand into the complete rule engine. If validation is not authorized or evidence is missing, the handoff says **code ready; runtime evidence pending** and lists the remaining gate. Merge readiness is separate; an unmerged prerequisite must be named with exact branch/SHA for the next lead.

Generate `docs/handoffs/stage-02-first-path.md` from that future actual state, update `CURRENT_HANDOFF.md` and `STATUS.md`, and display **START A NEW CHAT — Stage 2: first complete path** with its generated prompt. That next prompt must target import → stored observations → one computed finding → evidence detail; it must not invent the future checkpoint now.
...
Create codex/part-1-foundation and build the installable FastAPI backend, React/Vite frontend, pinned dependencies, scoped SQLite inventory/schema, small deterministic seed and API-backed inventory view. Implement explicit unseeded/readiness/error states and document actual runtime/data/static paths and missing commands. Coordinate schema/locks centrally; parallelize only independent owned files/worktrees. Spencer owns Part 6 packaging, not app logic, and can prepare alongside us. PART6_READY stays false until its real prerequisites exist.
...
Stop at the Stage 1 pushed checkpoint. Generate the actual Stage 2 handoff and paste-ready prompt from current branch/SHA, implemented behavior, dependencies and remaining gates. Update CURRENT_HANDOFF.md and STATUS.md and highlight START A NEW CHAT — Stage 2: first complete path. Keep this chat for assigned foundation fixes; do not start Stage 2 here unless I ask.
```
