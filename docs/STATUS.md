...
The lead owns eventual application main merges after required evidence. [Stage 5's approved preparation handoff](handoffs/stage-05-kickoff.md) separates an isolated local application E2E gate from Spencer's later portable package/recipient gate. A bounded local execution question is pending through the data-owner task; it is **not yet authorization**. Current no-execution limits remain in force, and no acceptance worker is assigned. Do not repeat the pending permission question.
