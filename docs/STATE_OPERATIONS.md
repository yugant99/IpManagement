...
**Source implementation; runtime evidence pending.** These commands consume the shared schema-v4/legacy-version contract published by Stage 4 at `16160806e91023a29eae501b824c6b43e5e56d87`. The existing state algorithm is unchanged; this follow-up updates CLI help and documentation only. No application, state command, test or build was run. This document supplies core behavior for Spencer's packaging/runbook; it does not change his owned files or declare `PART6_READY=yes`. See the earlier [schema compatibility handoff](handoffs/part-1-state-schema-compat.md) for state implementation history and [scheduling contract](SCHEDULING_CONTRACT.md) for the pending scheduler integration.
...
`backup --output PATH` copies the **entire SQLite main database** using `sqlite3.Connection.backup`, not selected rows or a raw copy of an open database file. It therefore includes inventory, original seed metadata, all Stage 2 import envelopes/receipts/raw and typed records/coverage, saved calculation runs and the findings embedded in their JSON. Schema v3's allocation requests, audit events, exceptions and report preset, plus v4's `schedule_status` and `schedule_operations`, are included when present. Other tables in the same database are also copied; code/assets, files outside SQLite and attached external databases are outside the snapshot. In particular, the `IPAM_SYNTHETIC_FEED_DIR` assets and packaged producer code must be supplied separately. Actual round-trip preservation remains unverified.
...
Restoring a v4 snapshot returns the whole database to its snapshot state: schedule enabled/interval/next-due values, feed/cycle cursor, status and committed replay records move together with the scenario clock, evidence, saved runs and audit. It neither keeps a newer pre-restore cursor nor clears the restored replay/settings independently. Manual operation keys committed after the snapshot are absent afterward; retry deduplication applies only to history retained in the restored database.

Restore itself never starts a timer. The scheduling contract requires a later service startup to make at most one overdue attempt from an enabled restored schedule, then schedule forward; it also requires visible refusal of missing/incompatible pinned feed assets rather than resetting restored state. Scheduler lifecycle/restart behavior remains pending source integration and runtime evidence at this document's pinned schema checkpoint.
...
Application identity and required tables come from shared `require_schema`. State validation uses the store owner's `MIGRATABLE_SCHEMA_VERSIONS` (currently v1, v2 and v3) and `SCHEMA_VERSION` (currently v4). A declared legacy version is checked with `require_schema(version=that_version)`; current and all other inputs go through the current-version check, so unknown versions are refused. Version 4 additionally requires both schedule tables and the `schedule_status` singleton; legacy checks do not require those new tables. There is no separate historical-version list or table policy in state operations.

This applies to backup sources, restore inputs/candidates, existing databases preserved before restore, and reset targets. A restored v1/v2/v3 database keeps its original version and returns `migration_required: true`; stop the service and use the existing explicit `python -m ipam_demo migrate` before normal current-runtime use. Current-v4 restore returns `migration_required: false`. State operations never advance a schema themselves, and normal startup remains restricted to the current schema. An uninitialized but structurally valid database stays uninitialized after backup/restore.

The additive v4 migration creates a disabled schedule with a six-hour interval, cycle index zero, no next due time and no committed operation records. It does not acquire/import evidence, create a reconciliation run or start a timer. An existing v4 store is not reinitialized by migration. Actual migration/preservation outcomes remain unverified.
...
Pending evidence includes v1/v2/v3/v4 snapshot round-trips and saved-ID/content preservation (including populated workflow/audit/exception/preset and schedule/replay tables); invalid/foreign/unsupported/corrupt input refusal; original preservation on copy/replace/permission failures; active-service refusal; WAL/rollback-journal behavior; reset scope; replay of backup/restore/reset failures; legacy restore then explicit migration; replacement of a legacy target with a v4 snapshot while retaining its original; restored schedule/replay/restart and external-asset refusal behavior; and untouched unrelated files. No tests were added, and no state command, application install, migration, build, smoke/browser check or infrastructure operation was executed here. Schema table availability alone does not establish populated workflow, audit or scheduler persistence evidence.
