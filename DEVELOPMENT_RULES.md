...
**Authorized exception, 2026-09-19:** the user requested Main Lead 2.0 to replace the original lead. Follow `docs/handoffs/main-lead-2.0.md`; receiver `01a0bb80-cf0d-7f60-8e46-1e825f42d276` is registered, and the outgoing task is reference-only. This exception changes lead identity, not worker ownership or execution permissions. The rule below still applies to ordinary implementation-stage transitions.

Follow `docs/PROJECT_OVERSIGHT.md` and `docs/CHAT_STAGES.md`. The original project-lead chat stays the overseer; fresh stage chats are implementation workers. A worker notices its checkpoint, commits/pushes, writes its lane report and drafts the next prompt. Show **READY FOR PROJECT-LEAD REVIEW — Stage N** with exact SHA/PR, evidence and gaps. The persistent lead reviews and coordinates acceptance/merge, updates global `STATUS.md`/`CURRENT_HANDOFF.md`, then shows **START A NEW IMPLEMENTATION CHAT — Stage N: title** with the accepted checkpoint, handoff link and complete copyable prompt. Workers do not self-accept or overwrite global status. The user should not reconstruct context. Keep old worker chats for assigned feature fixes; fixes to merged work use a new branch. No chat per commit and no automatic task creation/archive. A separately configured read-only heartbeat reports meaningful progress; it never implements, merges or grants runtime/infrastructure permission.
