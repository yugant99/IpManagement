...
The original project-lead chat remains the overseer across stages. Workers report **READY FOR PROJECT-LEAD REVIEW — Stage N**, with exact SHA/PR, evidence/gaps and a draft next prompt. The lead owns global status, acceptance/merge coordination and the final **START A NEW IMPLEMENTATION CHAT** prompt. Follow `docs/PROJECT_OVERSIGHT.md` and `docs/CHAT_STAGES.md`; do not rotate project ownership or let workers declare global completion. Use isolated worker worktrees, never switch another worker's checkout, and keep previous worker chats for assigned fixes.
